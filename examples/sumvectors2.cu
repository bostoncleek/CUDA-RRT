...
// paralle threads = N blocks * M threads/block

// Quadro M1000M
// Max threads per block:  1024
// Max thread dimensions:  (1024, 1024, 64)
// Max grid dimensions:  (2147483647, 65535, 65535)
#define NUMBER_BLOCKS 65535
...
{
  // each thread starts at different index
  int tid = threadIdx.x + blockIdx.x * blockDim.x;

  // Loop ensures we do not write more data than what we want to
  while(tid < N)
  {
    c[tid] = a[tid] + b[tid];
    // increment by the total number of running threads
...
int main()
{
  ...
  startTime = clock();
  addGPU<<< NUMBER_BLOCKS, THREADS_PER_BLOCK >>>( dev_a, dev_b, dev_c );
  endTime = clock();
